...
class EditProfileVC: UIViewController, SendSelectedIconDelegate, SendSelectedChampDelegate {
...
extension EditProfileVC {
    @objc func confirmButtonPressed(_ sender: UIButton) {
        let updatedUser = UserInfo(email: user?.email ?? "", userName: (userNameTextField.text ?? user?.userName) ?? "", tier: tierButton.titleLabel?.text ?? "", position: positionButton.titleLabel?.text ?? "", profilePhoto: selectedImage ?? "Default", mostChampion: selectedChamp ?? ["None", "None", "None"], uid: user?.uid ?? "", iBlocked: user?.iBlocked ?? [""], youBlocked: user?.youBlocked ?? [""])
    ...
    @objc func mostChampButtonPressed() {
        let chooseChampVC = ChooseChampVC()
