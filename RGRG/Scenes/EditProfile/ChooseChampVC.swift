...
#warning("챔피언 이름 위치 향후 치울 것")
class ChooseChampVC: UIViewController {
...
// SearchBarController
extension ChooseChampVC: UISearchResultsUpdating {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        filteringSearchResult(searchController.searchBar.text ?? "")
    }
    
    func filteringSearchResult(_ searchText: String) {
        filteredChamp = champNameKor.filter { $0.contains(searchText) }
        imageCollectionView.reloadData()
    }
}

// CollectionViewController
extension ChooseChampVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func setupCollectionView() {
        imageCollectionView.register(ChampionCell.self, forCellWithReuseIdentifier: "cell")
        imageCollectionView.dataSource = self
        imageCollectionView.delegate = self
    }
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if filteredChamp.count == 0 {
            return champName.count
        } else {
            return filteredChamp.count
        }
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? ChampionCell else {
            return UICollectionViewCell()
        }
        if filteredChamp.count != 0 {
            cell.iconImage.image = UIImage(named: champName[champNameKor.firstIndex(of: filteredChamp[indexPath.row]) ?? 0])
        } else {
