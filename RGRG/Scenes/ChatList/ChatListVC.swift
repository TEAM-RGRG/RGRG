...
extension ChatListVC {
    override func viewDidLoad() {
        ...
        FBStoreManager.shared.loadWholeChannels()
        FBStoreManager.shared.updateChannelsStatus { value in
...
extension ChatListVC {
    func task(tag: Int) {
        ...
        FBUserManager.shared.getUserInfo { [weak self] user in
            guard let self = self else { return }
            currentUser = user

            guard let currentUser = currentUser else { return }

            FBStoreManager.shared.loadChannels(collectionName: "channels", filter: currentUser.uid) { channel, _ in
