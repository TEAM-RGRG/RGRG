...
class PartyCreateVC: UIViewController {
    var user: UserInfo?
...
extension PartyCreateVC {
    func task() {
        if let user = user {
            ...
            let party = PartyInfo(champion: [user.mostChampion[0], user.mostChampion[1], user.mostChampion[2]], content: infoTextView.text ?? "", date: FBStoreManager.shared.dateFormatter(value: Date.now), hopePosition: hopePosition, profileImage: user.profilePhoto, tier: user.tier, title: partyNameTextField.text ?? "", userName: user.userName, writer: user.userName, position: user.position)
            
            Task {
                if tag == 1 {
                    await FBPartyManager.shared.addParty(party: party) { party in
                        print("### 업로드 된 :: \(party)")
                    ...
                    if thread != nil {
                        await FBPartyManager.shared.updateParty(party: party, thread: thread ?? "n/a") { data in
                            print("##### 여기 실행되어야함")
...
extension PartyCreateVC: UITextFieldDelegate {
...
extension PartyCreateVC: UITextViewDelegate {
