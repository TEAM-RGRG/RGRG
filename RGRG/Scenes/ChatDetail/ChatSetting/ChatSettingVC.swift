...
extension ChatSettingVC {
    ...
    @objc func tappedExitView() {
        ...
        if thread != nil {
            FBStoreManager.shared.deleteChannel(thread: thread ?? "", completion: {
...
extension ChatSettingVC {
    func showBenAlert() {
        ...
        let confirmAlert = UIAlertAction(title: "차단", style: .destructive, handler: { _ in
            if self.thread != nil {
                FBStoreManager.shared.deleteChannel(thread: self.thread ?? "n/a", completion: {
                    guard let presentingViewController = self.presentingViewController as? UINavigationController else { return }
                    let vc = ChatListVC()
                
                    UserBlockManager.shared.blockUser(uid: self.chatUserID ?? "n/a")
                    presentingViewController.popViewController(animated: true)
